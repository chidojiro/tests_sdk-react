...
} from "../ContextProvider/events";
import {
  UseSearchConfig,
  UseSearchCustomConfig,
  UseSearchParams,
  UseSearchResult,
} from "./types";
...
function useCustomSearch({
  pipeline,
  variables,
}: UseSearchCustomConfig): UseSearchResult {
  ...
  const [searchOutput, setSearchOutput] = useState<
    Omit<UseSearchResult, "searching">
  >({
    search: searchFn,
    error: null,
    redirects: {},
...
function useNormalSearch({
  ...
  return {
    ...
    results,
    search,
    searchInstant: searchInstant as any,
    searching,
    error,
  };
}
