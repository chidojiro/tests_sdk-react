...
const updateState = (
  ...
  if (responseValues) {
    suggestions = (
      (responseValues.get(config.qSuggestionsParam) || "") as string
    )
      .split(",")
      .filter((s) => s.length > 0)
...
const responseUpdatedListener = (
  variables: Variables,
  prevState: ProviderPipelineState,
  response: Response
) => {
  const {
    config,
...
const valuesUpdatedListener = (
  ...
  console.log("variables.get", variables.get());
...
const ContextProvider: React.FC<SearchProviderValues> = ({
  ...
  const autocompleteVariables = useRef(
    autocompleteProp?.variables ?? new Variables()
  );
  ...
  const searchFn = useCallback(
    (key: "search" | "autocomplete") =>
      (inputQuery?: string, override = false) => {
        ...
        const setSearchingState =
          key === "autocomplete" ? setAutocompleteSearching : setSearching;
        const timer =
          key === "autocomplete" ? searchAutocompleteTimer : searchTimer;
        setSearchingState(true);
        setSearchState((state) => ({
          ...state,
          query: inputQuery ?? state.query,
        }));
        const { pipeline, variables } =
          func as Required<ProviderPipelineConfig>;
        const { config } = state;
  ...
  useEffect(() => {
    ...
    unregisterFunctions.push(
      autocomplete.current.pipeline.listen(
        EVENT_RESPONSE_UPDATED,
        (response: Response) => {
  ...
  const clear = useCallback(
    (key: "search" | "autocomplete") =>
      (vals?: { [k: string]: string | undefined }) => {
        const func = key === "autocomplete" ? autocomplete.current : search;
        const { pipeline, variables } =
          func as Required<ProviderPipelineConfig>;
  ...
  const handlePaginate = useCallback(
    (page: number) => {
      const { pipeline, variables } =
        search as Required<ProviderPipelineConfig>;
  ...
  const handleResultClicked = useCallback(
    (args: { token: string; values: ResultValues }) =>
      search.pipeline.emitResultClicked(args),
    []
  );
