...
const collator = new Intl.Collator(
  !isSSR() ? window.navigator.language : "en-US",
  {
    numeric: true,
  }
);
...
const sortComparitor =
  (byLabel, asc) =>
  ([labelLeft, valueLeft], [labelRight, valueRight]) => {
...
export const sortOptions = (
  ...
  return list.sort(
    (
      { label: labelLeft, count: valueLeft },
      { label: labelRight, count: valueRight }
    ) =>
      sortComparitor(byLabel, asc)(
        [labelLeft, valueLeft],
...
export const filterItems = (
  list: Item[],
  {
    ...
    includes,
    prefixFilter,
  }: { includes?: string[]; excludes?: string[]; prefixFilter?: string }
): Option[] => {
  ...
  if (prefixFilter) {
    filteredList = filteredList
      ...
      .map(({ value, count }) => ({
        label: value.replace(prefixFilter, ""),
        count,
        value,
      }));
  }
